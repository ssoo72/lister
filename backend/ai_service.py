...
class AICompanyInfoService:
    ...
    async def get_company_info(self, company_name: str) -> Dict[str, Optional[str]]:
        ...
        try:
            prompt = f"""
以下の企業について、2025年時点の最新情報に基づいて、日本の就職活動で役立つ情報をJSON形式で提供してください。
実在する企業の場合は可能な限り正確な最新情報を、不明な場合は業界標準に基づいた妥当な推測を記載してください。
...
{{
  ...
  "salary": "大卒初任給の目安（2025年度実績または予測、月給、例: 25万円、30万円など。大手企業は25-30万円程度が標準）",
  "website_url": "企業の公式ウェブサイトURL（推測可能な場合）"
}}

注意：初任給は近年上昇傾向にあり、大手企業では25万円以上が一般的です。
不明な項目は null としてください。
"""
